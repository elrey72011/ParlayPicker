...
def _enrich_with_kalshi_safe(df: pd.DataFrame) -> tuple[pd.DataFrame, str | None]:
    ...
    try:
        ...
        future.cancel()
        return df, f"Kalshi enrichment timed out (>{KALSHI_ENRICH_TIMEOUT_SECONDS}s) â€” skipped."
...
_NO_BET_STAGE_REASONS = {
    "game_already_started": "â±ï¸ Game already started â€” live odds, not pre-game lines",
    "baseline_guardrail": "Market priced better â€” negative EV at this price",
    "actionable_threshold": "Failed the confidence threshold",
    "min_win_probability_floor": "Below the 55% win-probability floor",
    "empirical_proven_losing_bucket": "Proven-losing bucket â€” history says pass",
    "empirical_tier_overlay": "Small-sample bucket â€” not enough history to trust",
...
def _compose_model_probability(out: pd.DataFrame) -> tuple[pd.Series, pd.Series, pd.Series]:
    ...
    if is_broken_ml.any():
        ...
        logger.warning(f"âš ï¸ Trapped broken XGBoost scores (0.19063-0.19064) for matchups: {broken_matchups}. Discarding ML predictions and forcing Statistical Fallback.")
...
def _recompute_consensus_from_kalshi(df: pd.DataFrame, require_ml: bool = False) -> pd.DataFrame:
    ...
    if is_broken_ml.any():
        broken_matchups = out.loc[is_broken_ml, "matchup_id"].unique() if "matchup_id" in out.columns else []
        logger.warning(f"âš ï¸ Consensus Step: Trapped broken XGBoost scores (0.19063-0.19064) for matchups: {broken_matchups}.")
    ...
    if decimal_odds.isna().all():
        logger.warning("âš ï¸ All odds are missing - using default 1.91 (-110 equivalent)")
    ...
    # run_analysis_pipeline applied â€” the card then rebuilds on the raw model over-lean
    ...
    # is more likely â€” that is Disagrees, not Agrees, regardless of the probability gap.
...
def _merge_kalshi_into_analysis(analysis_df: pd.DataFrame, best_picks_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _mk_matchup_id(df: pd.DataFrame) -> pd.Series:
        home = df["home_team"].astype(str).str.lower().str.replace(r"[^a-z0-9\s]", "", regex=True).str.strip(…26405 tokens truncated…not bet ({len(research_prop_card)})",
                        expanded=False,
                    ):
                        st.caption(
                ...
                st.caption(
                    f"âš¾ MLB player props unavailable after retry ({_prop_err_type}). "
                ...
                st.caption(
                    f"âš¾ Pitcher-prop feed did not respond after retry ({_prop_stage}). "
                    "Run the slate again; no stale props were used."
                )
            else:
                st.caption("âš¾ No MLB player props cleared the edge bar today.")
            ...
            if len(compact_export) > 0:
                ...
                # ðŸ† Pick of the Day line so the export carries the day's single
                ...
                if _potd is not None:
                    _potd_cell = ws.cell(
                        ...
                        value=(
                            f"ðŸ† Pick of the Day: {_potd['pick']} "
                            f"({_potd['win_probability']:.1%} win prob, ${_potd['stake']:.2f}"
                            f"{', BELOW FLOOR â€” bet light' if _potd['below_floor'] else ''})"
                        ),
                    )
    ...
    with tab4:
        ...
        if base_parlays_df.empty:
            ...
            for idx, row in ranked.iterrows():
                ...
                tier_label = f" â€” {tier}" if tier else ""
