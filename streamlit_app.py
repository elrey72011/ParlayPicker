...
# required_display_cols moved to tab_master


# required_display_cols moved to top of with tab_master block
# -----------------
...
def apply_confidence_filter(df: pd.DataFrame, confidence_mode: str, show_low: bool) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    if not show_low:
        filtered = filtered # Temp disabled filter
...
def enrich_game_context(game: Dict[str, Any], league_key: str, api_key: Optional[str], sd_key: Optional[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        if api_client:
            ...
            # Force empty list safety and fuzzy matching
            games_api = []
...
# required_display_cols moved to top of file
...
with tab_master:
    ...
    required_display_cols = [
        ...
        "average_goals_conceded_home",
        "average_goals_conceded_away",
        "home_record_wins",
        "home_record_draws",
        "home_record_losses",
        "away_record_wins",
        ...
        "new_manager_bounce_home",
        "new_manager_bounce_away",
    ...
    if should_run:
        ...
        df_master_view_display = df_master_view_display.loc[:, ~df_master_view_display.columns.duplicated()].copy()
        ...
        top_df_display = top_df_display.loc[:, ~top_df_display.columns.duplicated()].copy()
...
with tab_shotgun:
    ...
    if "master_df" in st.session_state and not st.session_state["master_df"].empty:
        # Use the local df variable if available from the master run, otherwise fallback to session state
        # This ensures we have the most recent data even if session state hasn't fully propagated in this run context
