...
def implied_p_from_american(odds) -> float:
    def safe_american_to_decimal(odds) -> float | None:
        """
        Safely convert American odds to decimal odds.
        Returns None if odds are None, 0, or in the invalid (-100, 100) open interval.
        """
        try:
            if odds is None:
                return None
            ...
            # Valid American odds should be >= +100 or <= -100
            if abs(o) < 100:
                return None
...
with main_tab1:
    ...
    # Include historical window
    day_window = st.slider("Include events within ±N days", 0, 7, 0, 1, help="Leverage historical odds snapshots if your API plan supports them.")
# Sport Selection
    ...
    def is_in_window(iso_str) -> bool:
        try:
            ts_local = pd.to_datetime(iso_str, utc=True).tz_convert(tz)
            return abs((ts_local.date() - sel_date).days) <= day_window
        except Exception:
            return False
            ...
            with st.spinner("🧠 Analyzing markets with AI..."):
                ...
                for sport_idx, skey in enumerate(sports or APP_CFG["sports_common"]):
                    ...
                    for ev in (snap.get("events") or [])[:per_sport_events]:
                        if not is_in_window(ev.get("commence_time")): 
                            continue
                        ...
                        if inc_ml and "h2h" in mkts:
                            ...
                            if hp is not None and -750 <= hp <= 750:
                                ...
                                if ai_confidence >= min_ai_confidence:
                                    d_tmp = safe_american_to_decimal(hp)
                                    if d_tmp is None:
                                        pass
                                    else:
                                        d_tmp = safe_american_to_decimal(pr)
                                if d_tmp is None:
                                    continue
                                d_tmp = safe_american_to_decimal(pr)
                                if d_tmp is None:
                                ...
                                all_legs.append({
                                        ...
                                        "ai_edge": ai_edge,
                                        "d": safe_american_to_decimal(hp),
                                        "sentiment_trend": home_sentiment['trend']
                                    ,"d": d_tmp,"d": d_tmp,"d": d_tmp})
                            ...
                            if ap is not None and -750 <= ap <= 750:
                                ...
                                if ai_confidence >= min_ai_confidence:
                                    d_tmp = safe_american_to_decimal(ap)
                                    if d_tmp is None:
                                        pass
                                    else:
                                        d_tmp = safe_american_to_decimal(pr)
