...
def fair_prob_from_two_prices(home_odds: float, away_odds: float, side: str = "home") -> float:
    """
    No-vig normalization: convert two-sided American odds to fair probability.
    side: "home" or "away" – which side's prob to return.
    """
    try:
        ph = implied_p_from_american(home_odds)
        pa = implied_p_from_american(away_odds)
        total = ph + pa
        if total <= 0:
            return ph if side=="home" else pa
        ph_fair = ph / total
        pa_fair = pa / total
        return ph_fair if side == "home" else pa_fair
    except Exception:
        return implied_p_from_american(home_odds if side=="home" else away_odds)
...
def fetch_oddsapi_snapshot(api_key: str, sport_key: str) -> Dict[str, Any]:
    """
    Fetches odds from The Odds API and normalizes into a compact structure our app expects.
    Returns: {"events": [{
        "id": str, "commence_time": iso, "home_team": str, "away_team": str,
        "markets": {
            "h2h": {"home": {"price": int, "book": str}, "away": {"price": int, "book": str}},
            "spreads": [{"name": "home"|"away", "point": float, "price": int, "book": str}...],
            "totals": [{"name": "Over"|"Under", "point": float, "price": int, "book": str}...]
        }
    }...]}
    """
    ...
    for ev in data:
        try:
            ...
            # Some APIs call it away_team, ensure both exist
            # The Odds API uses "away_team"
            away = ev.get("away_team") or ev.get("away")
            eid = ev.get("id") or f"{home}_vs_{away}_{ev.get('commence_time')}"
            mkts = {"h2h": {}, "spreads": [], "totals": []}
            for b in (ev.get("bookmakers") or []):
                book_name = b.get("key") or b.get("title") or "unknown"
                for m in (b.get("markets") or []):
                    key = m.get("key")
                    outcomes = m.get("outcomes") or []
                    if key == "h2h":
                        h = next((o for o in outcomes if (o.get("name") or "").lower() in ["home","h","1", (home or "").lower()]), None)
                        a = next((o for o in outcomes if (o.get("name") or "").lower() in ["away","a","2", (away or "").lower()]), None)
                        # Fallback by comparing team names
                        if not h:
                            h = next((o for o in outcomes if (o.get("name") or "").lower() == (home or "").lower()), None)
                        if not a:
                            a = next((o for o in outcomes if (o.get("name") or "").lower() == (away or "").lower()), None)
                        if h and isinstance(h.get("price"), (int,float)):
                            cur = mkts.setdefault("h2h", {}).setdefault("home", {})
                            # Keep best (highest) price for underdogs; for favorites (negative), keep less negative (closer to zero)
                            pr = int(h["price"])
                            best = cur.get("price")
                            take = (best is None) or (pr > best)
                            if take:
                                mkts["h2h"]["home"] = {"price": pr, "book": book_name}
                        if a and isinstance(a.get("price"), (int,float)):
                            cur = mkts.setdefault("h2h", {}).setdefault("away", {})
                            pr = int(a["price"])
                            best = cur.get("price")
                            take = (best is None) or (pr > best)
                            if take:
                                mkts["h2h"]["away"] = {"price": pr, "book": book_name}
                    elif key == "spreads":
                        for o in outcomes:
                            nm = o.get("name")
                            pt = o.get("point")
                            ...
                            if nm is None or pt is None or pr is None:
                                continue
                            mkts["spreads"].append({"name": str(nm), "point": float(pt), "price": int(pr), "book": book_name})
                    elif key == "totals":
                        for o in outcomes:
                            nm = o.get("name")
                            pt = o.get("point")
                            ...
                            if nm is None or pt is None or pr is None:
                                continue
                            mkts["totals"].append({"name": str(nm), "point": float(pt), "price": int(pr), "book": book_name})
            events_out.append({
                "id": eid,
                    ...
                    for ev in (snap.get("events") or [])[:per_sport_events]:
                        ...
                        if inc_ml and "h2h" in mkts:
                            ...
                            if hp is not None and -750 <= hp <= 750:
                                base_prob = fair_prob_from_two_prices(hp, ap, side="home") if ap is not None else implied_p_from_american(hp)
                                ai_prob = base_prob
                            ...
                            if ap is not None and -750 <= ap <= 750:
                                base_prob = fair_prob_from_two_prices(hp, ap, side="away") if hp is not None else implied_p_from_american(ap)
                                ai_prob = base_prob
