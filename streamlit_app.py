...
# When college markets are selected (e.g., NCAAF) many sportsbooks only
# appear in the "us2" region on The Odds API. Query multiple regions so the
# app still receives lines even when a specific region is empty.
ODDS_API_REGION_PRIORITY: Tuple[str, ...] = ("us,us2", "us", "us2")
...
# Allow wider moneyline ranges so college blowouts (e.g., NCAAF) aren't discarded.
MAX_MONEYLINE_ABS_ODDS = 2500
...
def _is_reasonable_moneyline(price: Any, max_abs: float = MAX_MONEYLINE_ABS_ODDS) -> bool:
    """Return True when a moneyline price falls within an acceptable range."""
...
def _session_client_or_none(session_key: str, expected_type: Type[Any]):
    """Return a stored client only if it matches the expected type."""

    client = st.session_state.get(session_key)
    if client is None or isinstance(client, expected_type):
        return client

    try:
        unexpected = type(client).__name__
    except Exception:  # pragma: no cover - very defensive
        unexpected = str(type(client))
    logger.warning(
        "Session entry %s had unexpected type %s; resetting the cached client.",
        session_key,
        unexpected,
    )
...
def fetch_oddsapi_snapshot(
    api_key: str,
    sport_key: str,
    *,
    regions: Optional[Sequence[str]] = None,
) -> Dict[str, Any]:
    """Fetch odds from The Odds API, retrying across multiple regions if needed."""

    def _region_attempts() -> List[str]:
        if not regions:
            return list(ODDS_API_REGION_PRIORITY)
        ...
        # Try the combined string first (e.g., "us,us2") so a single call may
        # satisfy multiple coverage regions before falling back to the
        # individual entries.
        return [",".join(dedup)] + dedup

    region_values = _region_attempts()
    error_messages: List[str] = []

    warned_cache = st.session_state.setdefault('odds_api_region_warnings', set())
    ...
    for region_value in region_values:
        ...
        try:
            ...
            error_messages.append(f"{region_value}: timeout")
            continue
        except requests.exceptions.RequestException as exc:
            error_messages.append(f"{region_value}: {exc}")
            continue
        ...
        if not data or not isinstance(data, list):
            error_messages.append(f"{region_value}: empty response")
            continue
        ...
        if events:
            return {"events": events, "region": region_value}

    if sport_key not in warned_cache:
        sport_label = format_sport_label(sport_key)
        if error_messages:
            st.warning(
                f"Odds API request issues for {sport_label}: "
                + "; ".join(error_messages)
            )
        else:
            st.warning(
                f"No odds returned for {sport_label} across regions "
                + ", ".join(region_values)
            )
        warned_cache.add(sport_key)
...
def _format_leg_summary_for_export(
    leg: Dict[str, Any], *, timezone_label: Optional[str] = None
) -> str:
    """Create a concise textual summary for a leg when exporting parlays."""
    ...
    if not label:
        ...
        if home and away:
            ...
            label = leg.get('team') or "Selection"

    market = (leg.get('type') or leg.get('market') or '').strip()
    side = str(leg.get('side') or '').strip()
    descriptor_parts: List[str] = []
    if market:
        descriptor_parts.append(market)
    if side:
        descriptor_parts.append(f"({side})")
    descriptor = " ".join(descriptor_parts).strip()

    line_val = leg.get('line')
    if line_val is None:
        line_val = leg.get('point')
    if line_val is None:
        line_val = leg.get('handicap')

    line_text: Optional[str] = None
    if line_val is not None:
        try:
            line_text = f"{float(line_val):g}"
        except (TypeError, ValueError):
            line_text = str(line_val)

    decimal_price = _safe_float(leg.get('d'))
    american_price = leg.get('american_odds')
    if american_price is None and decimal_price:
        american_price = decimal_to_american(decimal_price)

    price_text: Optional[str] = None
    if american_price is not None:
        try:
            price_text = f"{int(american_price):+d}"
        except (TypeError, ValueError):
            price_text = str(american_price)
    elif decimal_price:
        price_text = f"{decimal_price:.3f}"

    kickoff_text: Optional[str] = None
    commence = leg.get('commence_time')
    if commence:
        try:
            kickoff_ts = pd.to_datetime(commence, utc=True)
            tz_name = timezone_label or 'UTC'
            try:
                tz_obj = pytz.timezone(tz_name)
            except Exception:
                tz_obj = pytz.UTC
            kickoff_text = kickoff_ts.tz_convert(tz_obj).strftime('%b %d %I:%M %p %Z')
        except Exception:
            kickoff_text = None
    ...
    if line_text:
        segments.append(f"Line {line_text}")
    if price_text:
        segments.append(f"Odds {price_text}")
    if kickoff_text:
...
def summarize_parlay_for_export(
    ...
    *,
    size_label: str,
    timezone_label: Optional[str] = None,
    within_size_rank: Optional[int] = None,
) -> Dict[str, Any]:
    """Produce a structured summary row for CSV/table exports."""

    if not parlay:
        return {}

    legs: List[Dict[str, Any]] = list(parlay.get('legs') or [])
    leg_summaries = [
        _format_leg_summary_for_export(leg, timezone_label=timezone_label)
        for leg in legs
    ]
    leg_summaries = [entry for entry in leg_summaries if entry]

    sport_keys = [leg.get('sport_key') for leg in legs if leg.get('sport_key')]
    sport_labels: List[str] = []
    for key in sorted(set(sport_keys)):
        try:
            sport_labels.append(format_sport_label(key))
        except Exception:
            sport_labels.append(str(key))

    theover_sources = parlay.get('theover_probability_sources') or []
    if isinstance(theover_sources, (set, tuple, list)):
        theover_sources_text = ", ".join(
            sorted(str(src) for src in theover_sources if src)
        )
    else:
        theover_sources_text = str(theover_sources) if theover_sources else ""

    legs_blob = " \n".join(
        f"{idx}. {text}" for idx, text in enumerate(leg_summaries, start=1)
    )

    summary = {
        "Parlay Size": size_label,
        "Within Size Rank": within_size_rank,
        "Leg Count": len(legs),
        "Sports": ", ".join(sport_labels) if sport_labels else None,
        "Decimal Odds": _safe_float(parlay.get('d')),
        "AI Probability": _safe_float(parlay.get('p_ai')),
        "Market Probability": _safe_float(parlay.get('p')),
        "AI Confidence": _safe_float(parlay.get('ai_confidence')),
        "AI Score": _safe_float(parlay.get('ai_score')),
        "AI Edge": _safe_float(parlay.get('ai_edge')),
        "AI EV": _safe_float(parlay.get('ev_ai')),
        "Market EV": _safe_float(parlay.get('ev_market')),
        "Profit ($100 stake)": _safe_float(parlay.get('profit')),
        "Kalshi Factor": _safe_float(parlay.get('kalshi_factor')),
        "Kalshi Boost": _safe_float(parlay.get('kalshi_boost')),
...
def build_best_bets_per_game(
    ...
    *,
    ...
    def dig(mapping, path, default=None):
        """Lightweight nested lookup that tolerates missing keys/indices."""
        ...
        try:
            for token in tokens:
                if current is None:
                    return default
                if '[' in token and token.endswith(']'):
                    name, index_str = token[:-1].split('[', 1)
                    if name:
    ...
    for sport_key in sport_keys:
        ...
        for event in filtered_events:
            ...
            if "h2h" in mkts:
                ...
                if _is_reasonable_moneyline(home_price):
                    home_price_raw = home_price
                    home_price_val = _safe_float(home_price)
                    base_prob = implied_p_from_american(home_price_val)
                    ai_prob = base_prob
                ...
                if _is_reasonable_moneyline(away_price):
                    away_price_raw = away_price
                    away_price_val = _safe_float(away_price)
                    base_prob = implied_p_from_american(away_price_val)
                    ai_prob = base_prob
                    ai_confidence = baseline_confidence
                    ai_edge = 0.0
                    ml_prob = None
...
def format_parlay_leg_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """Create a readable parlay leg table with consistent ordering and formatting."""
    ...
    ordered_df = df[existing + remaining]
    ...
    for col in percent_like:
        if col in formatted.columns:
            formatted[col] = formatted[col].map(_normalize_percent)
...
def render_parlay_section_ai(
    ...
    for i, row in enumerate(rows, start=1):
        ...
        try:
            ...
            logger.debug("Failed to refresh theover.ai blends for parlay legs", exc_info=True)
            ...
            if theover_result is not None:
                ...
                if isinstance(theover_result, dict):
                    ...
                    if isinstance(probability, (int, float)):
                        leg['theover_probability'] = probability
                        leg['theover_probability_delta'] = delta if isinstance(delta, (int, float)) else None
...
if not _session_client_or_none('apisports_hockey_client', APISportsHockeyClient):
...
if not _session_client_or_none('apisports_basketball_client', APISportsBasketballClient):
    ...
    basketball_client = _session_client_or_none('apisports_basketball_client', APISportsBasketballClient)
            ...
            with st.spinner("🧠 Analyzing markets with AI..."):
                ...
                if theover_ml_data is not None or theover_spreads_data is not None or theover_totals_data is not None:
                    try:
                        ...
                        # Do not block parlays if optional theover.ai processing fails
                        pass
                ...
                def _collect_parlay_export_rows(
                    label: str, combos: List[Dict[str, Any]]
                ) -> None:
                    for idx, parlay in enumerate(combos, start=1):
                        summary = summarize_parlay_for_export(
                            parlay,
                            size_label=label,
                            timezone_label=user_timezone_label,
                            within_size_rank=idx,
                        )
                        if summary:
                ...
                with tab_2:
                    ...
                    try:
                        with st.spinner("Calculating optimal 2-leg combinations..."):
                            ...
                            _collect_parlay_export_rows("2-leg", combos_2)
                ...
                with tab_3:
                    ...
                    try:
                        with st.spinner("Calculating optimal 3-leg combinations..."):
                            ...
                            _collect_parlay_export_rows("3-leg", combos_3)
                            render_parlay_section_ai(
                                "3-Leg AI Parlays",
                                combos_3,
                                theover_ml_data,
                ...
                with tab_4:
                    ...
                    try:
                        with st.spinner("Calculating optimal 4-leg combinations..."):
                            ...
                            _collect_parlay_export_rows("4-leg", combos_4)
                            render_parlay_section_ai(
                                "4-Leg AI Parlays",
                                combos_4,
                                theover_ml_data,
                ...
                with tab_5:
                    ...
                    try:
                        with st.spinner("Calculating optimal 5-leg combinations..."):
                            ...
                            _collect_parlay_export_rows("5-leg", combos_5)
                            render_parlay_section_ai(
                                "5-Leg AI Parlays",
                                combos_5,
                                theover_ml_data,
                ...
                if parlay_export_rows:
                    sorted_exports = sorted(
                        parlay_export_rows,
                        key=lambda row: (
                            _safe_float(row.get('AI Confidence')) or 0.0,
                            _safe_float(row.get('AI Score')) or 0.0,
                        ),
                        reverse=True,
                    )

                    ranked_exports: List[Dict[str, Any]] = []
                    for idx, row in enumerate(sorted_exports, start=1):
                        ranked = row.copy()
                        ranked['Rank'] = idx
                        ranked_exports.append(ranked)
...
# ═══════════════════════════════════════════════════════════════
# CSV UPLOAD → VERTEX-POWERED ANALYSIS WORKFLOW
# Upload theover.ai CSVs → Vertex analyzes everything → Best picks
# ═══════════════════════════════════════════════════════════════
...
def parse_theover_csv(uploaded_file):
    """
    Parse uploaded theover.ai CSV file
    Expected columns: Game, Home Team, Away Team, Sport, Date, etc.
    """
    try:
...
# ═══════════════════════════════════════════════════════════════
# STEP 2: VERTEX ANALYZES HISTORICAL GAMES
# ═══════════════════════════════════════════════════════════════

def vertex_analyze_historical_data(game, anthropic_api_key, historical_data=None):
    """
    Use Vertex AI to analyze historical game data and patterns
    """
    try:
        import anthropic
        client = anthropic.Anthropic(api_key=anthropic_api_key)
        ...
        prompt = f"""You are an expert sports analyst. Analyze this upcoming game with historical context:
...
ANALYSIS REQUIRED:
-----------------
1. Analyze recent head-to-head history between these teams
2. Consider home/away performance trends
3. Evaluate any relevant injuries, roster changes, or news
4. Assess betting line value and market sentiment
5. Consider sport-specific factors (rest days, travel, back-to-backs, etc.)
6. Identify any statistical edges or patterns

PROVIDE YOUR ANALYSIS AS JSON:
{{
  "home_win_probability": 55.5,
  "away_win_probability": 44.5,
  "confidence_level": 82,
  "risk_assessment": "Medium",
  "key_factors": [
    "Home team won last 3 meetings",
    "Away team on back-to-back",
    "Home team strong defensive rating"
  ],
  "historical_edge": "Home team historically performs well in this matchup",
  "betting_recommendation": "Home team moneyline",
  "value_rating": 8.5,
  "sentiment": "Bullish on home team",
  "concerns": ["Away team star player questionable", "Home team on short rest"],
  "final_pick": "Home Team",
  "reasoning": "Strong historical matchup advantage and home court in favorable spot"
}}
...
CRITICAL: Respond ONLY with valid JSON. No markdown, no explanations outside JSON."""
        ...
        st.warning(f"Vertex analysis error for {game['home_team']}: {str(e)[:100]}")
        return None
...
def run_ml_predictions(game, ml_predictor):
    """
    Run ML predictions on game
    """
    try:
        prediction = ml_predictor.predict_game(
            game['home_team'], 
            game['away_team']
        )
...
def run_sentiment_analysis(game, sentiment_analyzer):
    """
    Analyze sentiment for game
    """
    try:
        ...
        return {
            ...
            'sentiment_sources': result.get('sources', [])
        }
    except Exception as e:
        return {
...
# ═══════════════════════════════════════════════════════════════
# STEP 5: COMPREHENSIVE ANALYSIS POWERED BY VERTEX
# ═══════════════════════════════════════════════════════════════

def comprehensive_vertex_analysis(game, ml_result, sentiment_result, anthropic_api_key):
    """
    Vertex AI synthesizes ALL data for final winning probability
    """
    try:
        import anthropic
        client = anthropic.Anthropic(api_key=anthropic_api_key)
        
        prompt = f"""You are the FINAL DECISION MAKER for sports betting analysis. Synthesize all available data:
...
- ML Home Win Prob: {ml_result['ml_home_prob']:.1f}%
- ML Away Win Prob: {ml_result['ml_away_prob']:.1f}%
- ML Confidence: {ml_result['ml_confidence']:.1f}%

SENTIMENT ANALYSIS:
- Sentiment Score: {sentiment_result['sentiment_score']:.1f}
...
YOUR TASK:
Combine ML prediction, sentiment, betting lines, and your historical knowledge to produce the MOST ACCURATE winning probability.

Consider:
1. Does ML align with your historical knowledge?
2. Does sentiment confirm or contradict ML?
3. Are betting lines suggesting value or trap?
4. What do you know about these teams' recent form?
5. Any injuries, coaching changes, or situational factors?

PROVIDE FINAL ANALYSIS AS JSON:
{{
  "final_home_win_probability": 58.5,
  "final_away_win_probability": 41.5,
  "overall_confidence": 87,
  "risk_level": "Low",
  "recommended_bet": "Home Team Moneyline",
  "bet_confidence": "High",
  "value_assessment": 8.5,
  "key_insights": [
    "ML and sentiment align strongly",
    "Historical matchup favors home team",
    "Betting line offers value"
  ],
  "edge_analysis": "5.2% edge over implied probability",
  "final_verdict": "STRONG HOME TEAM PLAY",
  "stars": 5
}}
...
Respond ONLY with valid JSON."""
        ...
        st.warning(f"Comprehensive analysis error: {str(e)[:100]}")
        return None
...
def run_complete_analysis(games, ml_predictor, sentiment_analyzer, anthropic_api_key):
    """
    Run complete analysis pipeline on all games
    """
    results = []
    ...
    for idx, game in enumerate(games):
        status_text.write(f"🔍 Analyzing {idx+1}/{total_games}: {game['away_team']} @ {game['home_team']}")
        
        # Step 1: Vertex Historical Analysis
        status_text.write(f"   🧠 Vertex analyzing historical data...")
        vertex_historical = vertex_analyze_historical_data(game, anthropic_api_key)
        
        # Step 2: ML Predictions
        status_text.write(f"   🤖 Running ML predictions...")
        ml_result = run_ml_predictions(game, ml_predictor) if ml_predictor else {
            'ml_home_prob': 50.0,
            'ml_away_prob': 50.0,
            'ml_confidence': 50.0,
            'ml_model': 'N/A'
        ...
        status_text.write(f"   📰 Analyzing sentiment...")
        sentiment_result = run_sentiment_analysis(game, sentiment_analyzer) if sentiment_analyzer else {
            'sentiment_score': 50.0,
            'sentiment_label': 'Neutral',
            'sentiment_sources': []
        }
        
        # Step 4: Comprehensive Vertex Synthesis
        status_text.write(f"   🎯 Vertex synthesizing final analysis...")
        final_analysis = comprehensive_vertex_analysis(
        ...
        result = {
            ...
            # Vertex Historical
            'Vertex Historical Edge': vertex_historical.get('historical_edge', 'N/A')[:100] if vertex_historical else 'N/A',
            'Vertex Initial Confidence': f"{vertex_historical.get('confidence_level', 0):.1f}%" if vertex_historical else 'N/A',
            
            # ML
            'ML Home %': f"{ml_result['ml_home_prob']:.1f}%",
            'ML Away %': f"{ml_result['ml_away_prob']:.1f}%",
            'ML Confidence': f"{ml_result['ml_confidence']:.1f}%",
            
            # Sentiment
            'Sentiment': sentiment_result['sentiment_label'],
            'Sentiment Score': f"{sentiment_result['sentiment_score']:.1f}",
            
            # Final Vertex Analysis
            'FINAL Home Win %': f"{final_analysis.get('final_home_win_probability', 0):.1f}%" if final_analysis else 'N/A',
            'FINAL Away Win %': f"{final_analysis.get('final_away_win_probability', 0):.1f}%" if final_analysis else 'N/A',
            'Overall Confidence': f"{final_analysis.get('overall_confidence', 0):.1f}%" if final_analysis else 'N/A',
            'Risk Level': final_analysis.get('risk_level', 'N/A') if final_analysis else 'N/A',
            'Recommended Bet': final_analysis.get('recommended_bet', 'N/A') if final_analysis else 'N/A',
            'Value Rating': f"{final_analysis.get('value_assessment', 0):.1f}" if final_analysis else 'N/A',
            'Stars': final_analysis.get('stars', 0) if final_analysis else 0,
            'Final Verdict': final_analysis.get('final_verdict', 'N/A') if final_analysis else 'N/A',
            'Key Insights': ', '.join(final_analysis.get('key_insights', [])) if final_analysis else 'N/A',
    ...
    # Create DataFrame and sort by confidence/stars
    df = pd.DataFrame(results)
    
    # Sort by stars and confidence
...
st.write("---")
st.write("---")
st.header("📊 CSV Upload → Vertex-Powered Analysis")

st.info("""
**Workflow:**
1. 📤 Upload theover.ai CSV file
2. 🧠 Vertex analyzes historical data for each game
3. 🤖 ML predictions run
4. 📰 Sentiment analysis runs
5. 🎯 Vertex synthesizes everything for FINAL winning probabilities
6. 📥 Download comprehensive analysis

**Powered primarily by Vertex AI for maximum accuracy!**
""")

# CSV Upload
st.subheader("Step 1: Upload CSV")
uploaded_file = st.file_uploader(
    "Upload theover.ai CSV file",
    type=['csv'],
    help="CSV should contain: Home Team, Away Team, Sport, Date, Odds, etc."
)
...
if uploaded_file is not None:
    ...
    if games:
        st.success(f"✅ Parsed {len(games)} games from CSV")
        
        # Show preview
        with st.expander("📋 View Uploaded Games"):
        ...
        # Run Analysis
        st.subheader("Step 2: Run Vertex-Powered Analysis")
        ...
        col1, col2 = st.columns(2)
        with col1:
            use_ml = st.checkbox("Include ML Predictions", value=True if ml_predictor else False)
        with col2:
            use_sentiment = st.checkbox("Include Sentiment Analysis", value=True if sentiment_analyzer else False)
        
        if not anthropic_key:
            st.error("❌ Anthropic API key required. Add it in the sidebar.")
        else:
            if st.button("🚀 Run Complete Analysis", type="primary", key="run_analysis_btn"):
                with st.spinner("🔍 Running comprehensive Vertex-powered analysis..."):
                    results_df = run_complete_analysis(
                        games,
                        ml_predictor if use_ml else None,
                        sentiment_analyzer if use_sentiment else None,
                        anthropic_key
                    )
                
                if not results_df.empty:
                    st.success(f"✅ Analysis complete! Analyzed {len(results_df)} games")
                    ...
                    with col1:
                        st.metric("Total Games", len(results_df))
                    with col2:
                        five_star = len(results_df[results_df['Stars'] >= 5])
                        st.metric("5-Star Picks", five_star)
                    with col3:
                        high_conf = len(results_df[results_df['Overall Confidence'].str.replace('%', '').astype(float) >= 80])
                    ...
                    # Results table
                    st.write("### 🎯 Best Picks (Ranked by Stars & Confidence)")
                    st.dataframe(results_df, use_container_width=True)
                    ...
                    # Top picks detail
                    st.write("---")
                    st.write("### 🌟 Top 5 Picks Detail")
                    
                    top_5 = results_df.head(5)
                    for idx, row in top_5.iterrows():
                        with st.expander(f"⭐ Rank {row['Rank']}: {row['Game']} - {row['Stars']}★"):
                            col1, col2 = st.columns(2)
                            
                            with col1:
                                st.write("**Probabilities:**")
                                st.write(f"- Final Home Win: {row['FINAL Home Win %']}")
                                st.write(f"- Final Away Win: {row['FINAL Away Win %']}")
                                st.write(f"- ML Home: {row['ML Home %']}")
                                st.write(f"- ML Away: {row['ML Away %']}")
                                
                                st.write("**Confidence:**")
                                st.write(f"- Overall: {row['Overall Confidence']}")
                                st.write(f"- ML: {row['ML Confidence']}")
                                st.write(f"- Risk: {row['Risk Level']}")
                            
                            with col2:
                                st.write("**Recommendation:**")
                                st.write(f"- Bet: {row['Recommended Bet']}")
                                st.write(f"- Value: {row['Value Rating']}/10")
                                st.write(f"- Verdict: {row['Final Verdict']}")
                                
                                st.write("**Sentiment:**")
                                st.write(f"- {row['Sentiment']} ({row['Sentiment Score']})")
                            
                            st.write("**Key Insights:**")
                            st.write(row['Key Insights'])
