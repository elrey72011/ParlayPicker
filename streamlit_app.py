...
with tab_shotgun:
    ...
    if "master_df" in st.session_state and not st.session_state["master_df"].empty:
        ...
        if len(candidates_tight) >= 2:
            ...
            # Ensure we use the relaxed pool if it offers sufficient plays or more than tight
            if len(candidates_normal) >= 2 or len(candidates_normal) > len(candidates_tight):
