    ...
    try:
        ...
        if use_new_matching:
            ...
            if res_winner.matched and not res_spread.matched and raw_matches.get("_meta", {}).get("status") == "matched":
                ...
                # Task Requirement: Explicitly set status = market_type_missing and available = False
                res_spread.reason = "market_type_missing"
                res_spread.kalshi_available = False
                res_spread.matched = False
                # Do NOT fallback to winner probability (incorrect for spread analysis)

            if res_winner.matched and not res_total.matched and raw_matches.get("_meta", {}).get("status") == "matched":
                # We have a game match, but no total markets.
                # Task Requirement: Explicitly set status = market_type_missing and available = False
                res_total.reason = "market_type_missing"
                res_total.kalshi_available = False
                res_total.matched = False
                # Do NOT fallback to winner probability (incorrect for total analysis)
        ...
        with st.spinner("🔄 Analyzing Markets..."):
            ...
            for idx, g in enumerate(games_to_process):
                try:
                    ...
                    rows_count_before = len(accumulated_rows)
                    ...
                    logger.error(f'❌ CRITICAL ERROR processing game {idx}: {g.get("away_team", "Unknown")} @ {g.get("home_team", "Unknown")}')
                    logger.exception(e)

                    # EMERGENCY FALLBACK: Ensure at least one row exists to prevent empty dataframe
                    try:
                        emergency_row = {
                            "league": g.get("league"),
                            "Home": g.get("home_team"),
                            "Away": g.get("away_team"),
                            "Game": f"{g.get('away_team')} @ {g.get('home_team')}",
                            "Commence (UTC)": g.get("commence_time_iso_utc") or g.get("commence_time"),
                            "Market": "Moneyline",
                            "Pick": None,
                            "Pick_Confidence": "LOW",
                            "confidence_reason": f"Processing Error: {str(e)}",
                            "Pick_Reason_Short": "Processing Error",
                            "Warnings": f"exception_fallback;{str(e)[:50]}",
                            "final_probability": 0.5,
                            "Best Overall Prob": 0.5,
                            "stats_quality": "ERROR"
                        }
                        accumulated_rows.append(emergency_row)
                        logger.info(f"✅ Created EMERGENCY fallback row for {g.get('home_team')}")
                    except:
                        logger.error("Failed to create emergency fallback row")
