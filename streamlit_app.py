    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if data:
                ...
                st.sidebar.warning(f" {sport_key}: {error[:50]}")
...
def american_to_decimal_safe(odds) -> Optional[float]:
    """
    Safe AmericanDecimal conversion.
...
def render_saved_parlay_tracker(clients: Dict[str, Any], timezone_label: str) -> None:
    ...
    with st.expander("View saved parlays and outcomes", expanded=False):
        ...
        with actions_col2:
            clear_clicked = st.button(" Clear all tracked parlays", key="clear_tracked_parlays")
        ...
        for entry in tracked:
            ...
            with st.expander(f"{parlay_label}  {status_display}"):
                ...
                if st.button(" Remove from tracker", key=f"remove_tracked_{entry.get('parlay_id')}"):
...
def render_sidebar_controls() -> Dict[str, Any]:
    ...
    if st.session_state.get('news_api_key'):
        ...
        sidebar.caption(" Using neutral fallback sentiment")
    ...
    for sport_key, cfg in SPORTSDATA_CONFIG.items():
        ...
        if st.session_state.get(session_key):
            ...
            sidebar.caption(
                f" Add your SportsData.io {cfg['label']} key to enrich live metrics and ML features"
            )
    ...
    try:
        ...
        sidebar.warning(
            " Sports selector failed to format options. Using default sports list instead."
        )
...
def summarize_parlay_for_export(
    ...
    *,
    ...
    summary = {
        ...
        "theover Avg ": _safe_float(parlay.get('theover_avg_delta')),
        ...
        "Kalshi Alignment ": _safe_float(parlay.get('kalshi_alignment_avg')),
        "Kalshi Alignment ||": _safe_float(parlay.get('kalshi_alignment_abs_avg')),
    ...
    *,
    ...
    for leg in legs:
        ...
        if not edge_candidates and sportsdata_delta is not None:
            edge_candidates.append(("SportsData ", sportsdata_delta))
    ...
    for event_id, options in legs_by_event.items():
        ...
        row = {
            ...
            'theover  pp': best_option['theover_delta'] * 100 if best_option['theover_delta'] is not None else None,
            ...
            'SportsData  pp': best_option['sportsdata_delta'] * 100 if best_option['sportsdata_delta'] is not None else None,
            'Kalshi Prob %': best_option['kalshi_prob'] * 100 if best_option['kalshi_prob'] is not None else None,
            'Kalshi  pp': best_option['kalshi_delta'] * 100 if best_option['kalshi_delta'] is not None else None,
...
def render_parlay_section_ai(
    ...
    for i, row in enumerate(rows, start=1):
        ...
        prob = row['p_ai']
        if prob < 0.25:
            prob_warning = ""  # Warning for low probability
        elif prob < 0.35:
        ...
        if conflict_count:
            ...
            theover_segments.append(f" {conflict_count} {label}")
        ...
        if prob_count:
            ...
            theover_segments.append(f"ML {delta_pct:+.1f}pp ({prob_count} {legs_label})")
        ...
        if kalshi_legs > 0:
            ...
            if kalshi_factor > 1.05:
                kalshi_boost = f" |  {kalshi_legs} Kalshi +{(kalshi_factor-1)*100:.0f}%"
            ...
            if align_avg:
                kalshi_boost += f" {align_avg*100:+.1f}pp vs ML"
        ...
        if apisports_legs:
            ...
            icon_sequence = "".join(
                sport_icon_lookup.get(sport, "") for sport in sorted(set(apisports_sports))
            ) or ""
        ...
        if kalshi_legs_with_data > 0:
            ...
            if live_data_legs_with_data:
                st.markdown("###  Live Data Influence (API-Sports + SportsData.io):")

                if live_data_sports:
                    icons = " ".join(
                        sport_icon_lookup.get(sport, '') for sport in sorted(set(live_data_sports))
                    )
                    st.caption(
        ...
        if theover_prob_count:
            ...
            base_msg = f"ML probabilities blended for {theover_prob_count} {legs_label} (avg {delta_pct:+.1f}pp vs AI)."
            ...
            if bonus_pct > 0.1:
                ...
                st.warning(
                    f" **theover.ai Conflict:** {bonus_pct:.0f}% penalty  {base_msg}"
                )
            else:
            ...
            if bonus_pct > 0:
                ...
                st.warning(
                    f" **theover.ai Conflict:** {bonus_pct:.0f}% penalty "
        ...
        with comp_col2:
            ...
            if abs(prob_diff) > 1:
                if prob_diff > 0:
                    st.success(f" Sentiment boosted by {prob_diff:.1f}%")
                else:
                    st.warning(f" Sentiment reduced by {abs(prob_diff):.1f}%")
        ...
        for j, leg in enumerate(row["legs"], start=1):
                ...
                if isinstance(theover_result, dict):
                    ...
                    if isinstance(probability, (int, float)):
                        ...
                        if isinstance(delta, (int, float)):
                            ...
                            theover_display = f"{signal} {pick} ({prob_pct:.1f}% | {delta*100:+.1f}pp)"
                        else:
            ...
            if isinstance(apisports_details, dict) and apisports_details:
                ...
                if trend:
                    icon = {'hot': '', 'cold': '', 'neutral': ''}.get(trend, '')
            ...
            if isinstance(sportsdata_details, dict) and sportsdata_details:
                ...
                if trend_sd:
                    icon_sd = {'hot': '', 'cold': '', 'neutral': ''}.get(trend_sd, '')
        ...
        if has_theover:
            st.caption(
                "**theover.ai %** = ML probability from theover.ai; **theover ** = percentage-point change versus the AI model before blending."
            )
        ...
        if has_theover:
            ...
            with col_legend1:
                st.caption(" = Matches theover.ai pick |  = Conflicts with theover.ai |  = theover.ai data available")
            with col_legend2:
                if theover_matches > 0:
                    st.success(f" {theover_matches} leg(s) match theover.ai recommendations")
                if theover_conflicts > 0:
                    st.warning(f" {theover_conflicts} leg(s) conflict with theover.ai recommendations")
        ...
        if kalshi_available > 0:
            ...
            with col_k1:
                st.caption("**Legend:**  = Confirms |  = Kalshi higher |  = Kalshi lower |  = Strong value |  = Contradiction")
            with col_k2:
                ...
                if kalshi_contradicts > 0:
                    st.warning(f" {kalshi_contradicts} leg(s) contradicted by Kalshi")
            ...
            if kalshi_details:
                ...
                if total_kalshi_edge > 0.10:
                    ...
                    st.warning(f" **OVERPRICED WARNING**: Kalshi thinks this parlay is overpriced. Sportsbooks may be offering poor value.")
                ...
                if kalshi_score > 15:
                    ...
                    st.warning(" **KALSHI SKEPTICAL** - Prediction market doesn't support this parlay. Bet with caution or skip.")
...
if is_vertex_ai_enabled():
        ...
        with st.spinner("Running comprehensive AI analysis... Consolidating all data sources..."):
            try:
                ...
                if odds_client:
                    ...
                    st.warning(" The Odds API not configured")
                ...
                if not all_games:
                    ...
                    if not results_df.empty:
                        ...
                        st.warning(" No results from analysis. Try adjusting your filters.")
    ...
    if use_kalshi:
        ...
        **Kalshi Validation Benefits:**
        ...
        -  Flags contradictions when markets disagree
        ...
        **Tip:** Check Tab 4 to see what Kalshi markets are actually available right now.
        ...
        with st.expander(" Understanding Kalshi Market Coverage"):
            coverage_text = """
    ...
    if 'vertex_results' in st.session_state and st.session_state.get('vertex_analysis_complete'):
        ...
        st.warning(" Vertex AI analysis not run yet. Best bets will use legacy method. Run 'Vertex AI Master Analysis' first for better results!")
    ...
    if vertex_available_for_parlays:
        ...
        st.warning(" Run 'Vertex AI Master Analysis' first for AI-powered parlay optimization with better probabilities")
        ...
        try:
            ...
            with st.spinner(" Analyzing markets with AI..."):
                ...
                if parlay_export_rows:
                    ...
                    st.download_button(
                        " Download all parlay projections (CSV)",
                        data=full_csv,
...
with main_tab2:
    ...
    with col_api1:
        odds_key = st.session_state.get('api_key', "") or os.environ.get("ODDS_API_KEY", "")
        if not odds_key:
            st.warning(" Odds API key not configured. Please set it in the Sports Betting tab.")
                        ...
                        for result in analysis_results:
                            ...
                            with st.expander(f"{trend_icon} {team} - {sentiment['trend'].upper()} ({sentiment['score']:+.2f})"):
                                ...
                                with col_info2:
                                    ...
                                    if next_game:
                                        location = " Home" if next_game['location'] == 'home' else " Away"
...
with main_tab3:
    ...
    if not api_key:
        st.warning(" Please enter your Odds API key in the 'Sports Betting Parlays' tab first")
        st.stop()
        ...
        if selected_game_idx is not None:
            ...
            if selection is not None and st.button(" Add to Parlay", type="secondary"):
                ...
                if len(st.session_state['custom_legs']) >= 4:
                    st.warning(" Maximum 4 legs allowed. Remove a leg to add another.")
                else:
                    ...
                    if duplicate:
                        st.warning(" This leg is already in your parlay")
                    else:
    ...
    if st.session_state['custom_legs']:
        ...
        if len(st.session_state['custom_legs']) >= 2:
                ...
                with st.spinner(" Running AI/ML analysis..."):
                    try:
                        ...
                        if ai_prob > market_prob * 1.1:
                            ...
                            insights.append(f" AI sees {((1-ai_prob/market_prob)*100):.0f}% worse chance than market")
                        
                        if ai_metrics['confidence'] > 0.7:
                            insights.append(" High AI confidence across all legs")
                        elif ai_metrics['confidence'] < 0.5:
                            insights.append(" Low AI confidence - consider alternative picks")
                        ...
                        if positive_sentiment == len(analyzed_legs):
                            ...
                            insights.append(" No picks have positive sentiment")
            ...
            st.info(" Add at least 2 legs to analyze your parlay")
        
        # Clear parlay button
        if st.button(" Clear All Legs", type="secondary"):
            st.session_state['custom_legs'] = []
            st.rerun()
    else:
    ...
    **Making Decisions:**
    ...
    -  Caution: Negative AI EV or Low Confidence
...
with main_tab4:
    ...
    analysis_mode = st.radio(
        ...
        [" Browse Kalshi Sports Markets", " Compare with Sportsbooks", " Find Arbitrage Opportunities"],
        horizontal=True
    )
    ...
    elif analysis_mode == " Compare with Sportsbooks":
        st.subheader(" Kalshi vs Sportsbook Odds Comparison")
    ...
    with st.expander(" Important Considerations"):
        st.markdown("""
            ...
            if not games:
                ...
                for raw_game in games:
                    ...
                    with col_home:
                        ...
                        if home.trend:
                            icon = {'hot': '', 'cold': '', 'neutral': ''}.get(home.trend, '')
                    ...
                    with col_away:
                        ...
                        if away.trend:
                            icon = {'hot': '', 'cold': '', 'neutral': ''}.get(away.trend, '')
                    ...
                    for raw_game in sd_games:
                        ...
                        def _render_sd_team(column, label, team):
                            with column:
                                ...
                                if team.trend:
                                    icon = {'hot': '', 'cold': '', 'neutral': ''}.get(team.trend, '')
...
if 'best_bets_df' in st.session_state and st.session_state['best_bets_df'] is not None:
    ...
    if st.button(" Auto-Enrich Best Bets", type="primary", key="auto_enrich"):
        ...
        with st.spinner(" Enriching best bets with theover.ai data..."):
                        ...
                        if fuzzy_match(home_team, t_home) and fuzzy_match(away_team, t_away):
                            ...
                            if market_matches:
                                ...
                                try:
                                    ...
                                    enriched['theover  pp'] = f"{theover_prob - ai_prob:+.1f}"
                                except:
                                    enriched['theover  pp'] = ''
            ...
            preview_cols = ['Game', 'Market', 'AI Prob %', 'theover.ai %', 'theover  pp']
    ...
    st.warning(" No best bets found. Generate best bets first using the sections above.")
    ...
    if st.button(" Enrich CSV with All Data", type="primary"):
        ...
        for idx, row in incomplete_df.iterrows():
            ...
            try:
                ...
                st.warning(f" Could not parse: {game}")
                enriched_rows.append(enriched)
                continue
            ...
            if pd.isna(row['SportsData Prob %']) or row['SportsData Prob %'] == '':
                ...
                enriched['SportsData  pp'] = 52.0 - row['AI Prob %']
