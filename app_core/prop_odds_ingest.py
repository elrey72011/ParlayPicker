...
def parse_strikeout_props(
    ...
    """Extract per-pitcher strikeout over/under lines from one event-odds payload.

    Returns one row per pitcher: ``{pitcher, line, over_odds, under_odds, book,
    home_team, away_team}``. Uses the first book in ``book_priority`` that prices the
    market (falls back to any book that does). Pitchers missing a complete over+under
    pair at a single line are skipped.
    """
    ...
    for book in ordered:
        market = next(
            (m for m in book.get("markets", []) if m.get("key") == STRIKEOUT_MARKET_KEY),
            None,
        )
        if not market:
            continue
...
def fetch_strikeout_props(client: Any, sport_key: str, event_id: str) -> list[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            return []
        return parse_strikeout_props(resp.json())
    except Exception:
        return []
