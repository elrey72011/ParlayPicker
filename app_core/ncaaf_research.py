...
import numpy as np
from app_core.ncaaf_history import build_dataset, checkpoint_bytes, timestamp

FEATURES = ["home_ppg", "away_ppg", "home_oppg", "away_oppg",
            "home_yards_per_game", "away_yards_per_game", "neutral_site"]
PROTOCOL = {"version": "ncaaf-research-v1", "train": 2023, "calibrate": 2024, "evaluate": 2025,
            "ridge_alpha": 10.0, "minimum_rows_per_split": 50, "minimum_prior_games": 3,
...
def _eligible(r):
    if r.get("neutral_site") not in (True, False) or not isinstance(r.get("neutral_site"), bool):
        return False
    try:
        return (r.get("scoring_features_available") is True
                and min(r[k] for k in ("home_prior_games", "away_prior_games", "home_yards_games", "away_yards_games")) >= 3
                and all(math.isfinite(float(r[k])) for k in FEATURES))
    except (KeyError, TypeError, ValueError):
        return False


def prepare(features, targets):
    lookup = {t["game_id"]: t for t in targets}
    if len(lookup) != len(targets) or len({r["game_id"] for r in features}) != len(features):
        raise ValueError("Duplicate game IDs in research inputs")
    splits, coverage = {}, []
    for year in (2023, 2024, 2025):
        candidates = [r for r in features if r["season"] == year]
        rows = []
        for r in candidates:
            if not _eligible(r):
                continue
            t = lookup.get(r["game_id"])
            if not t or not all(math.isfinite(float(t[k])) for k in ("home_margin", "total_points")):
                raise ValueError("Missing or invalid research target")
            if timestamp(r["kickoff"]) is None:
                raise ValueError("Invalid research kickoff")
            rows.append({**r, "y_margin": t["home_margin"], "y_total": t["total_points"]})
        rows.sort(key=lambda r: (timestamp(r["kickoff"]), r["game_id"]))
        if len(rows) < PROTOCOL["minimum_rows_per_split"]:
            raise ValueError(f"At least 50 eligible games are required for {year}")
        splits[year] = rows
        coverage.append({"season": year, "available": len(candidates), "eligible": len(rows), "excluded": len(candidates) - len(rows)})
    if not (timestamp(splits[2023][-1]["kickoff"]) < timestamp(splits[2024][0]["kickoff"])
            and timestamp(splits[2024][-1]["kickoff"]) < timestamp(splits[2025][0]["kickoff"])):
...
def run_research(state):
    ...
    if audit["requests_remaining"] or state["years"] != [2023, 2024, 2025]:
        raise ValueError("Complete the fixed 2023Ã¢â‚¬â€œ2025 collection before research evaluation")
    if any(s["games_with_both_team_stats"] != s["usable_games"] for s in audit["seasons"]):
        raise ValueError("Resolve missing team statistics before research evaluation")
    splits, coverage = prepare(features, targets)
    artifact = fit(splits[2023], splits[2024])
    frozen_hash = digest(artifact)  # freeze before holdout evaluation
    metrics, predictions = evaluate(artifact, splits[2025])
    report = {"protocol": PROTOCOL, "artifact_hash": frozen_hash,
              "checkpoint_hash": hashlib.sha256(checkpoint_bytes(state)).hexdigest(),
              "coverage": coverage, "holdout_metrics": metrics,
              "limitations": [v for v in audit["limitations"] if not v.startswith("No historical odds,")] + ["No live wagering approval is included.", "2025 is now evaluated; future tuning needs a new evaluation period.",
                  "Winner accuracy is not an approved-bet hit rate. No historical odds or returns were evaluated."],
              "production_eligible": False}
...
def markdown_report(result):
    ...
    lines = ["# NCAAF research evaluation", "", "Train: 2023. Calibrate: 2024. Evaluate: 2025. Research only.", "",
    ...
    lines += ["", "Errors are mean absolute errors in points. Winner accuracy is across all eligible games, not approved wagers.", "", "Artifact: " + report["artifact_hash"], "", "## Limitations", ""]
