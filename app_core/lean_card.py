...
see, per game: the model's side, its confidence, and an honest tier â€”

  * BET   â€” the pick the system would actually stake (Actionable: a real, priced edge).
  * LEAN  â€” the model has a positive-EV side but below the stake bar, and it is not fading
            Kalshi. A read worth knowing; NOT a proven +EV bet. Bet at your own risk.
  * AVOID â€” negative EV at the price, or the model is fading consensus (Disagrees). The
...
def classify_lean_tier(status: object, eff_ev: object, consensus: object,
    ...
    AVOID if its CALIBRATED win probability fails to beat the break-even price â€” the model's
...
def score_best_picks_rows(best_picks_df: pd.DataFrame, *, calibration: object = _UNSET,
    ...
    # under:Agrees ~61%) isn't crushed below break-even by the pooled curve â€” same number the
    ...
    # Empirical edge = bucket-aware calibrated win minus break-even. This â€” NOT model EV â€” is
    ...
    # A started game is never playable at ANY size â€” pre-game lines are stale
...
def build_all_games_lean_card(best_picks_df: pd.DataFrame, *, calibration: object = _UNSET,
    ...
    # Rank by empirical edge (bucket-proven), not model Win%/EV â€” the latter is anti-informative.
...
def attach_play_stakes(card: pd.DataFrame, unit: float = 1.0) -> pd.DataFrame:
    ...
    positive-EV production decision:
      ...
      BET    2.0u (or the pick's own Kelly stake if larger â€” a real edge)
      LEAN   0u
      AVOID  0u
