...
def render_parlay_section_ai(title, rows):
    ...
    for i, row in enumerate(rows, start=1):
        ...
        with st.expander(
            ...
            st.dataframe(pd.DataFrame(legs_data), , hide_index=True)
...
_day_window = st.slider("Include events within ±N days", 0, 7, 0, 1, help="Leverage historical odds snapshots.")", 
        ...
        with st.spinner(f"Analyzing {pp_sport} player props..."):
            ...
            for i, entry in enumerate(best_entries, start=1):
                ...
                with st.expander(
                    ...
                    st.dataframe(pd.DataFrame(picks_data), , hide_index=True)
...
def safe_american_to_decimal_safe(odds) -> float | None:
    """
    Safely convert American odds to decimal odds.
    Returns None if odds are None, 0, or in the invalid open interval (-100, 100).
    """
    try:
        if odds is None:
            return None
...
def american_to_decimal_safe(odds) -> float | None:
    """
    Wrapper used across the app; rely on safe conversion and skip invalids.
    """
    return safe_american_to_decimal_safe(odds)
