...
def test_probability_display_order_and_rendered_lists(tmp_path):
    ...
    funcs='\n'.join(line for line in html.splitlines() if line.startswith(('function probabilityOrder(', 'function lockedRows(', 'function render(){', 'function renderProps(', 'function currentResearchRows(', 'function renderGameBoardView(', 'function syncBoardViewButtons(')))
    script="""
...
const BOARD_VIEW_TITLES={overall:'Overall Best Picks',sides:'Sides',totals:'Totals'};let activeBoardView='overall';
const renderApprovedPicks=()=>{},renderBoardSummary=()=>{},renderTopPicks=()=>{},renderLockedPicks=()=>{},renderParlays=()=>{},renderResults=()=>{};
...
render();assert.equal(tables.length,2);assert.ok(tables.every(t=>t.ranked&&t.names[0]==='High'));
...
for(const view of ['overall','sides','totals']){activeBoardView=view;tables=[];render();assert.equal(tables.length,2);assert.ok(tables.every(t=>t.names.join(',')==='High,Low,Missing'));}
