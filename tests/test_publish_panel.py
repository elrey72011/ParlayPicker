...
def test_locks_render_before_embedded_preview(monkeypatch):
    ...
    monkeypatch.setattr(st,'iframe',lambda *args,**kwargs: calls.append('preview'))
    monkeypatch.setenv('PARLAYPICKER_PUBLISH_TOKEN','test-only-publish-token')
    at=AppTest.from_function(app).run()
...
def test_preview_iframe_preserves_generated_html(monkeypatch):
    ...
    at.text_input(key='publication_token').set_value('test-only-publish-token').run()
    assert not at.exception
    frame=at.get('iframe')[0].proto
    assert frame.srcdoc==at.session_state['publication_preview']['html']
    assert frame.scrolling and not frame.src
