...
def test_pick_details_selection_survives_filter_reruns_and_layout_changes():
    app = AppTest.from_function(_navigation_app).run()
    assert not app.exception
    assert app.session_state["main_navigation"] == "Today"
    def browser_rerun():
        ...
        assert tab_id
        states.widgets.add(id=tab_id, string_value="Pick Details")
        app._run(states)
    ...
    browser_rerun()
    assert not app.exception
    assert app.session_state["main_navigation"] == "Pick Details"
    app.number_input[0].set_value(0.06)
    browser_rerun()
    assert not app.exception
    assert app.caption[-1].value == "Pick Details"
...
def _legacy_app():
    import streamlit as st
    from streamlit_app import _render_main_tabs
    st.session_state["main_navigation"] = "Best Picks"
    _render_main_tabs()


def test_previous_navigation_migrates_to_workspace():
    app = AppTest.from_function(_legacy_app).run()
    assert not app.exception
    assert app.session_state["main_navigation"] == "Workspace"
