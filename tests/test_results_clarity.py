...
def test_browser_defaults_saved_filters_yesterday_and_overlap(tmp_path):
    ...
    initialization = html.split("const data=",1)[1].split("const el=",1)[0]
...
function run(saved,locked=true){
 ...
 const context={Date:clock,Intl,document:{getElementById(id){if(id==='board-data')return {textContent:JSON.stringify({results:rows})};return nodes[id]||(nodes[id]=new E());},createElement:()=>({})},localStorage:{getItem:()=>saved?JSON.stringify(saved):null},el:(tag,text)=>({tag,text}),fmt:(n,p)=>p?(100*n).toFixed(1)+'%':n};
