...
def test_is_stale_schedule_handles_naive_base_and_aware_now():
    ...
    stale = sp.is_stale_schedule(base_df, bet_rows_df, now, max_age_days=14)
...
def test_is_stale_schedule_empty_base_with_populated_bets_is_stale():
    ...
    stale = sp.is_stale_schedule(base_df, bet_rows_df, pd.Timestamp("2026-02-01T00:00:00Z"), max_age_days=14)
...
def test_is_stale_schedule_mixed_invalid_dates_does_not_crash():
    ...
    now = pd.Timestamp("2026-03-06T12:00:00Z")
...
def test_run_analysis_pipeline_stale_base_falls_back_to_bet_rows(monkeypatch):
    ...
    assert diagnostics["base_stale"] is True
    assert diagnostics["base_max_date"] is not None
    assert diagnostics["bet_rows_max_date"] is not None
    assert diagnostics["now_utc"] is not None
