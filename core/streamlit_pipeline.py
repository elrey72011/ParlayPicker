...
PIPELINE_BUILD = "2026-08-16a-complete-export-run-traceability"
...
def _filter_preselection_line_integrity(
    ...
    home_ml_price = _numeric_series(pool, "game_home_ml_price")
    away_ml_price = _numeric_series(pool, "game_away_ml_price")
    has_two_way_moneyline = (
        home_ml_price.notna()
        & away_ml_price.notna()
        & home_ml_price.ne(away_ml_price)
    )
    home_is_moneyline_favorite = home_ml_price.lt(away_ml_price)
    selected_is_home = market.eq("spread_home")
    selected_is_moneyline_favorite = (
        (selected_is_home & home_is_moneyline_favorite)
        | (~selected_is_home & ~home_is_moneyline_favorite)
    )
    ...
    if diagnostics_out is not None:
        ...
        diagnostics_out["preselection_mlb_spread_orientation_fault_count"] = int(
            mlb_spread_orientation_fault.sum()
        )
...
def build_best_picks_df(analysis_df: pd.DataFrame, diagnostics_out: dict | None = None) -> pd.DataFrame:
    ...
    for idx in best.index:
        ...
        if (
            ...
            _orientation_args = (
                _mt_row,
                _line_for_orient,
                best.at[idx, "game_home_ml_price"]
                if "game_home_ml_price" in best.columns
                else pd.NA,
                best.at[idx, "game_away_ml_price"]
                if "game_away_ml_price" in best.columns
                else pd.NA,
            )
            if str(league).strip().upper() == "MLB":
                # MLB's standard +/-1.5 side follows even a narrow moneyline
                # favorite. The Texas -117/+115 regression sat just below the
                # generic 8-point ambiguity threshold and therefore bypassed the
                # old backstop despite a clearly reversed run-line pair.
    ...
    if not best.empty:
        ...
        strict_candidate_count = (
            best.groupby(strict_identity_key, dropna=False)["home_team"]
            .transform("size")
            .fillna(0)
        )
...
def _raw_book_odds_diag(row):
    """One verbatim string of every book's spread point/price pairs, moneyline prices,
    and total points. Keeping the spread price beside its signed point makes a reversed
    outcome binding visible in the export instead of presenting a plausible-looking line
    with the opposing side's price."""
    parts = []
    for bk in ("novig", "fanduel", "draftkings", "betmgm"):
        hp, ap = row.get(f"{bk}_home_point"), row.get(f"{bk}_away_point")
        hsp, asp = row.get(f"{bk}_home_price"), row.get(f"{bk}_away_price")
        ...
        if all(
            pd.isna(pd.to_numeric(v, errors="coerce"))
            for v in (hp, ap, hsp, asp, hml, aml, op, up)
        ):
            continue
        parts.append(
            f"{bk}: sp H={_fmt_odds_token(hp)}@{_fmt_odds_token(hsp)}"
            f"/A={_fmt_odds_token(ap)}@{_fmt_odds_token(asp)} "
...
def _standard_spread_consensus_quote(row, side: str):
    ...
    A standard book whose signed run line contradicts its own two-way moneyline is
    excluded. Multiple books can repeat the same upstream team-label reversal, so a
    raw majority is not sufficient evidence that the signed outcome is trustworthy.
    """
    if side not in {"home", "away"}:
        return pd.NA, pd.NA, None

    verified_by_point: dict[float, list[tuple[str, object]]] = {}
    unverified_by_point: dict[float, list[tuple[str, object]]] = {}
    for book in ("fanduel", "draftkings", "betmgm"):
        ...
        orientation_verified = pd.notna(home_ml) and pd.notna(away_ml) and float(home_ml) != float(away_ml)
        if orientation_verified:
            if pd.isna(home_point) or (
                (float(home_point) < 0.0) != (float(home_ml) < float(away_ml))
            ):
                continue
        key = round(float(point), 4)
        price = pd.to_numeric(row.get(f"{book}_{side}_price"), errors="coerce")
        target = verified_by_point if orientation_verified else unverified_by_point
        target.setdefault(key, []).append((book, price))

    by_point = verified_by_point or unverified_by_point
    if not by_point:
        return pd.NA, pd.NA, None
...
def _consistent_standard_spread_pair(row, side: str):
    """Return a corroborated, price-bound standard-book spread pair.

    At least two standard books must corroborate the complete signed pair. When a
    book also has a two-way moneyline, its negative run-line side must agree with
    its own favorite. This prevents two feeds carrying the same upstream reversal
    from outvoting the only correctly oriented book. Point and price always remain
    attached to the same quoted outcome; this helper never swaps prices.
    """
    if side not in {"home", "away"}:
    ...
    for book in ("fanduel", "draftkings", "betmgm"):
        ...
        if abs(float(home_point) + float(away_point)) > 1e-9:
            continue
        if (
            pd.notna(consensus_magnitude)
            and abs(abs(float(home_point)) - float(consensus_magnitude))
        ...
        if orientation_verified:
            if (float(home_point) < 0.0) != (float(home_ml) < float(away_ml)):
                continue
            verified_pairs.append(pair)
        else:
            unverified_pairs.append(pair)

    # Prefer independently verified quotes. If no standard book published a
    # moneyline, retain the earlier two-book signed-pair consensus as a fallback.
    pairs = verified_pairs or unverified_pairs
...
def _novig_spread_quote_for_favorite(row, requested_side: str, favorite_side: str):
    """Return the exact Novig signed outcome for the requested team side.

    The Odds API can attach Novig's two mirrored run-line outcomes to the opposite
    team names. Novig's own two-way moneyline independently identifies which team
    is the favorite. Only when both spread outcomes and prices form a complete,
    non-zero mirrored pair may the outcomes be rebound by sign. The signed point
    always keeps its original price, so the repair cannot manufacture a quote.

    Without a usable favorite, preserve the direct team-bound quote unchanged.
    """
    if requested_side not in {"home", "away"}:
        return pd.NA, pd.NA, False

    direct_point = pd.to_numeric(
        row.get(f"novig_{requested_side}_point"), errors="coerce"
    )
    direct_price = pd.to_numeric(
        row.get(f"novig_{requested_side}_price"), errors="coerce"
    )
    if favorite_side not in {"home", "away"}:
        if pd.notna(direct_point) and pd.notna(direct_price):
            return float(direct_point), float(direct_price), False
        return pd.NA, pd.NA, False

    quotes = {}
    for quoted_side in ("home", "away"):
        point = pd.to_numeric(
            row.get(f"novig_{quoted_side}_point"), errors="coerce"
        )
        price = pd.to_numeric(
            row.get(f"novig_{quoted_side}_price"), errors="coerce"
        )
        if pd.isna(point) or pd.isna(price):
            return pd.NA, pd.NA, False
        quotes[quoted_side] = (float(point), float(price))

    home_point, _ = quotes["home"]
    away_point, _ = quotes["away"]
    if (
        home_point == 0.0
        or away_point == 0.0
        or abs(home_point + away_point) > 1e-9
    ):
        return pd.NA, pd.NA, False

    requested_is_favorite = requested_side == favorite_side
    for quoted_side, (point, price) in quotes.items():
        if (point < 0.0) == requested_is_favorite:
            return point, price, quoted_side != requested_side
    return pd.NA, pd.NA, False
...
def _expand_live_odds_to_bet_rows(live_odds_df: pd.DataFrame, theover_rows: pd.DataFrame | None = None) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for idx, row in live_odds_processed.iterrows():
        ...
        opposing_price_suffixes = {
            "spread_home": "away_price",
            "spread_away": "home_price",
            "total_over": "under_price",
            "total_under": "over_price",
            "moneyline_home": "h2h_away_price",
            "moneyline_away": "h2h_home_price",
        }
        ...
        for market_type in candidate_markets:
            ...
            opposing_price_val = pd.to_numeric(
                row.get(f"novig_{opposing_price_suffixes[market_type]}"),
                errors="coerce",
            )
            market_dict["opposing_odds_american"] = (
                float(opposing_price_val) if pd.notna(opposing_price_val) else pd.NA
            )
            market_dict["opposing_odds_source"] = (
                "novig" if pd.notna(opposing_price_val) else "missing"
            )
            ...
            if market_type.startswith("spread"):
                ...
                if novig_team_binding_basis:
                    ...
                    novig_orientation_is_consistent = (
                        not novig_moneyline_favorite_side
                        or (
                            pd.notna(novig_home_point)
                            and (
                                (float(novig_home_point) < 0.0)
                                == (novig_moneyline_favorite_side == "home")
                            )
                        )
                    )
                    ...
                    opposing_side = "away" if side == "home" else "home"
                    (
                        novig_oriented_point,
                        novig_oriented_price,
                        novig_pair_reoriented,
                    ) = _novig_spread_quote_for_favorite(
                        row, side, novig_moneyline_favorite_side
                    )
                    (
                        novig_opposing_point,
                        novig_opposing_price,
                        novig_opposing_reoriented,
                    ) = _novig_spread_quote_for_favorite(
                        row, opposing_side, novig_moneyline_favorite_side
                    )
                    novig_oriented_pair_is_usable = (
                        not novig_outlier
                        and pd.notna(novig_oriented_point)
                        and pd.notna(novig_oriented_price)
                        and pd.notna(novig_opposing_point)
                        and pd.notna(novig_opposing_price)
                        and abs(
                            float(novig_oriented_point)
                            + float(novig_opposing_point)
                        )
                        <= 1e-9
                        and float(novig_oriented_price) != 0.0
                        and float(novig_opposing_price) != 0.0
                        and float(novig_oriented_price)
                        >= float(BEST_AVAILABLE_SPREAD_MIN_AMERICAN_ODDS)
                        and float(novig_opposing_price)
                        >= float(BEST_AVAILABLE_SPREAD_MIN_AMERICAN_ODDS)
                        and abs(float(novig_oriented_price)) <= 10000.0
                        and abs(float(novig_opposing_price)) <= 10000.0
                        and novig_pair_reoriented == novig_opposing_reoriented
                    )

                    standard_point = standard_price = standard_opposing_price = pd.NA
                    standard_book = None
                    standard_rebound = False
                    if novig_outlier or (
                        not novig_orientation_is_consistent
                        and not novig_oriented_pair_is_usable
                    ):
                        (
                            standard_point,
                            standard_price,
                            standard_opposing_price,
                            standard_book,
                        ) = _consistent_standard_spread_pair(row, side)
                        if pd.isna(standard_point):
                            (
                                standard_point,
                                standard_price,
                                standard_opposing_price,
                    ...
                    standard_pair_is_usable = (
                        pd.notna(standard_point)
                        and pd.notna(standard_price)
                        and pd.notna(standard_opposing_price)
                    )
                    if standard_pair_is_usable:
                        point_val = float(standard_point)
                        market_dict["odds_american"] = float(standard_price)
                        market_dict["opposing_odds_american"] = float(
                            standard_opposing_price
                        )
                        market_dict["odds_source"] = "odds_api"
                        market_dict["opposing_odds_source"] = standard_book
                        spread_line_source = (
                            f"{standard_book}_standard_spread_consensus"
                        )
                        rebound_suffix = (
                        ...
                        market_dict["orientation_source"] = (
                            ...
                            f"|standard_spread_consensus{rebound_suffix}"
                        )
                    elif novig_outlier:
                        point_val = pd.NA
                        ...
                        if novig_oriented_pair_is_usable:
                            point_val = float(novig_oriented_point)
                            market_dict["odds_american"] = float(
                                novig_oriented_price
                            )
                            market_dict["odds_source"] = "novig"
                            market_dict["opposing_odds_american"] = float(
                            ...
                            market_dict["opposing_odds_source"] = "novig"
                            if novig_pair_reoriented:
                                spread_line_source = "novig_moneyline_reoriented"
                                market_dict["orientation_source"] = (
                                    f"{orientation_source}|novig_moneyline_favorite"
                            ...
                            if pd.notna(standard_point):
                                ...
                                market_dict["orientation_source"] = (
                                    f"{market_dict.get('orientation_source', '')}"
                    ...
                    if league_str != "MLB":
                        if cb is not None and pd.notna(cb_point):
                            ...
                            if pd.notna(cb_price):
                                ...
                                cb_opposing_price = pd.to_numeric(
                                    row.get(
                                        f"{cb}_{'away' if side == 'home' else 'home'}_price"
                                    ),
                                    errors="coerce",
                                )
                                market_dict["opposing_odds_american"] = (
                                    float(cb_opposing_price)
                                    if pd.notna(cb_opposing_price)
                                    else pd.NA
                                )
                                market_dict["opposing_odds_source"] = (
                        ...
                        if pd.notna(consensus_point) and pd.notna(consensus_price):
                            ...
                            consensus_opposing_price = pd.to_numeric(
                                row.get(
                                    f"{consensus_book}_"
                                    f"{'away' if side == 'home' else 'home'}_price"
                                ),
                                errors="coerce",
                            )
                            market_dict["opposing_odds_american"] = (
                                float(consensus_opposing_price)
                                if pd.notna(consensus_opposing_price)
                                else pd.NA
                            )
                            market_dict["opposing_odds_source"] = (
                                consensus_book
                            ...
                            if novig_outlier or not novig_has_real_price:
                                ...
                                point_val = _derive_spread_away_line(row)
                # A spread is one two-sided market. If either selected or opposing
                # quote is missing, suspended, or outside the production price floor,
                # reject the pair before any fallback can manufacture -110. This
                # catches partial P2P markets such as TB -3.5 +111 / TEX +3.5 -100000
                # even when only one standard book is present and an outlier consensus
                # cannot be formed.
                existing_spread_rejection = str(
                    market_dict.get("odds_source", "")
                ).startswith("rejected_")
                if league_str == "MLB" and not existing_spread_rejection:
                    selected_spread_price = pd.to_numeric(
                        market_dict.get("odds_american"), errors="coerce"
                    )
                    opposing_spread_price = pd.to_numeric(
                        market_dict.get("opposing_odds_american"), errors="coerce"
                    )
                    spread_price_pair_is_usable = (
                        pd.notna(selected_spread_price)
                        and pd.notna(opposing_spread_price)
                        and float(selected_spread_price) != 0.0
                        and float(opposing_spread_price) != 0.0
                        and float(selected_spread_price)
                        >= float(BEST_AVAILABLE_SPREAD_MIN_AMERICAN_ODDS)
                        and float(opposing_spread_price)
                        >= float(BEST_AVAILABLE_SPREAD_MIN_AMERICAN_ODDS)
                        and abs(float(selected_spread_price)) <= 10000.0
                        and abs(float(opposing_spread_price)) <= 10000.0
                    )
                    if not spread_price_pair_is_usable:
                        (
                            fallback_point,
                            fallback_price,
                            fallback_opposing_price,
                            fallback_book,
                        ) = _consistent_standard_spread_pair(row, side)
                        fallback_rebound = False
                        if pd.isna(fallback_point) and spread_favorite_side:
                            (
                                fallback_point,
                                fallback_price,
                                fallback_opposing_price,
                        ...
                        fallback_pair_is_usable = (
                            pd.notna(fallback_point)
                            and pd.notna(fallback_price)
                            and pd.notna(fallback_opposing_price)
                            and float(fallback_price) != 0.0
                            and float(fallback_opposing_price) != 0.0
                            and float(fallback_price)
                            >= float(BEST_AVAILABLE_SPREAD_MIN_AMERICAN_ODDS)
                            and float(fallback_opposing_price)
                            >= float(BEST_AVAILABLE_SPREAD_MIN_AMERICAN_ODDS)
                            and abs(float(fallback_price)) <= 10000.0
                            and abs(float(fallback_opposing_price)) <= 10000.0
                        )
                        if fallback_pair_is_usable:
                            point_val = float(fallback_point)
                            market_dict["odds_american"] = float(fallback_price)
                            market_dict["opposing_odds_american"] = float(
                                fallback_opposing_price
                            )
                            market_dict["odds_source"] = "odds_api"
                            market_dict["opposing_odds_source"] = fallback_book
                            spread_line_source = (
                                f"{fallback_book}_standard_spread_consensus"
                            )
                            market_dict["orientation_source"] = (
                                f"{market_dict.get('orientation_source', '')}"
                                "|standard_spread_price_pair"
                                + (
                                    "|standard_spread_sign_rebound"
                                    if fallback_rebound
                                    else ""
                            ...
                            market_dict["opposing_odds_american"] = pd.NA
                            market_dict["odds_source"] = (
                                "rejected_live_spread_price"
                            )
                            market_dict["opposing_odds_source"] = "rejected"
                            spread_line_source = "rejected_live_spread_price"
                            market_dict["orientation_source"] = (
                                f"{market_dict.get('orientation_source', '')}"
                ...
                if pd.isna(point_val) or _novig_outlier:
                    ...
                    if cb is not None and pd.notna(cb_point):
                        ...
                        if pd.notna(cb_price):
                            market_dict["odds_american"] = float(cb_price)
                            opposing_total_side = "under" if t_side == "over" else "over"
                            cb_opposing_price = pd.to_numeric(
                                row.get(f"{cb}_{opposing_total_side}_price"),
                                errors="coerce",
                            )
                            market_dict["opposing_odds_american"] = (
                                float(cb_opposing_price)
...
def run_analysis_pipeline(
    ...
    # Final guardrail: patch ordinary missing quotes, but never resurrect a spread
    # that the orientation/outlier checks explicitly rejected. Rejected rows remain
    # visible for diagnostics with null EV/edge and cannot enter selection.
    missing_odds_mask = merged["odds_american"].isna()
    rejected_odds_mask = _string_series(merged, "odds_source").str.startswith(
        "rejected_", na=False
    )
    patchable_missing_odds_mask = missing_odds_mask & ~rejected_odds_mask
    missing_odds_count = int(patchable_missing_odds_mask.sum())
    if missing_odds_count > 0:
        ...
        patched_games = merged[patchable_missing_odds_mask][['home_team', 'away_team', 'market_type']].to_dict('records')
        ...
        merged.loc[patchable_missing_odds_mask, "odds_american"] = -110.0
        merged.loc[patchable_missing_odds_mask, "odds_source"] = _string_series(
            merged, "odds_source"
        ).where(
    ...
    # Remove vig using the actual complementary quote paired with the selected
    # point/price source. The former implementation looked up the opposing Novig
    # price by home/away label after spread reorientation and averaged the two
    # implied probabilities. That could reuse the selected quote as its own
    # opponent and made both sides non-complementary. A two-way no-vig probability
    # is selected_implied / (selected_implied + opposing_implied).
    paired_opposing_price = _numeric_series(merged, "opposing_odds_american")
    paired_opposing_implied = paired_opposing_price.apply(american_to_prob)
    paired_denominator = implied_prob + paired_opposing_implied
    paired_market_probability = (
        implied_prob / paired_denominator.where(paired_denominator > 0.0)
    ).clip(0.01, 0.99)

    # Fallback de-vig when a real paired quote is unavailable.
    synthetic_opposing_implied = (
        merged["odds_american"]
        .apply(get_opposing_odds_from_exchange)
        .apply(american_to_prob)
    )
    fallback_market_probability = (
        implied_prob / (implied_prob + synthetic_opposing_implied)
    ).clip(0.01, 0.99)
    merged["market_probability"] = paired_market_probability.where(
    ...
    if not merged.empty:
        selected_price = _numeric_series(merged, "odds_american")
        opposing_price = _numeric_series(merged, "opposing_odds_american")
        valid_odds_mask = selected_price.isna() | selected_price.between(
            -10000, 10000, inclusive="both"
        )
        valid_opposing_odds_mask = opposing_price.isna() | opposing_price.between(
            -10000, 10000, inclusive="both"
        )
        valid_prob_mask = merged["market_probability"].isna() | (
            (merged["market_probability"] >= 0.05)
            & (merged["market_probability"] <= 0.95)
        )

        invalid_price_mask = ~valid_odds_mask | ~valid_opposing_odds_mask
        invalid_probability_mask = ~valid_prob_mask
        rejected_value_mask = invalid_price_mask | invalid_probability_mask
        rejected_count = int(rejected_value_mask.sum())
        logger.info(
            "PIPELINE AUDIT: [3/9] Rows rejected by sanitization: %s",
            rejected_count,
        )
        logger.info(
            "PIPELINE AUDIT: [4/9] Rows dropped or excluded after sanitization: %s",
            rejected_count,
        )
        if rejected_count > 0:
            logger.warning(
                "Sanitization layer rejected %s row(s) with suspended/pathological "
                "prices or market probabilities; no synthetic -110 was created.",
                rejected_count,
            )
            price_rejection = invalid_price_mask & rejected_value_mask
            probability_rejection = invalid_probability_mask & ~price_rejection
            merged.loc[rejected_value_mask, "odds_american"] = pd.NA
            merged.loc[rejected_value_mask, "opposing_odds_american"] = pd.NA
            merged.loc[rejected_value_mask, "market_probability"] = pd.NA
            merged.loc[rejected_value_mask, "decimal_odds"] = pd.NA
            merged.loc[price_rejection, "odds_source"] = "rejected_live_price"
            merged.loc[probability_rejection, "odds_source"] = (
                "rejected_live_probability"
            )
            merged.loc[rejected_value_mask, "opposing_odds_source"] = "rejected"
            merged.loc[price_rejection, "line_source"] = "rejected_live_price"
            merged.loc[probability_rejection, "line_source"] = (
                "rejected_live_probability"
            )
            if "spread_line" in merged.columns:
                merged.loc[rejected_value_mask, "spread_line"] = pd.NA
            if "total_line" in merged.columns:
                merged.loc[rejected_value_mask, "total_line"] = pd.NA
            merged.loc[rejected_value_mask, "sanitized_value"] = True
